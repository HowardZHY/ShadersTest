...
vec3 sky_color = ivec3(180,150,255)/255.0;								
/*--------------------------------*/						
...
const vec3 moonlight2 = pow(normalize(vec3(0.8, 0.9, 1.4) * 0.03),vec3(3.0))*length(vec3(0.8, 0.9, 1.4) * 0.03);

return grad3*pow(L*(c*exp(d*Y)+A),1.0-rainStrength*0.6)*0.6*skyMult*sunVisibility + mix(vec3(0.8, 0.9, 1.4) * 0.03,moonlight2,1.-L2/2.0)*(L2+1.0)*moonVisibility;
}
...
void main() {
	...
	vec2 trCalc = min(abs(worldTime-vec2(23200.0,12000.0)),750.0);
	tr = max(min(trCalc.x,trCalc.y)/375.0-1.0,0.0);

const vec3 moonlight = vec3(1.6, 1.0, 1.0) * 0.001;
	/*--------------------------------*/
	gl_Position = ftransform();
	texcoord = (gl_MultiTexCoord0).xy;
	/*--------------------------------*/
	if (worldTime < 12000 || worldTime > 23200) {
	...
	float eyebright = max(eyeBrightnessSmooth.y/255.0-0.5/16.0,0.0)*1.2;
...
//1.03225806452
	...
	vec3 sun_ambient = bounced.w * (vec3(0.32,0.64,1.32)-rainStrength*vec3(0.12,0.32,1.08)) + sunlight*(bounced.x + bounced.z);
	...
	vec3 sun_ambient2 = bounced2.w * (vec3(0.32,0.64,1.32)-rainStrength*vec3(0.12,0.32,1.08)) + sunlight*(bounced2.x + bounced2.z);

	vec3 moon_ambient = (moonlight*3.6);

	vec3 moon_ambient2 = (moonlight*3.6);
	...
	vec3 sky_color = vec3(0.1, 0.32, 1.);
	...
	cloudColor = sunlight04 *sunVisibility*(1.0-rainStrength*0.97)*length(rawAvg) + rawAvg*0.7*(1.0-rainStrength*0.5) + 2.0*moonlight*moonVisibility*(1.0-rainStrength*0.96);
	cloudColor2 = 0.1*sunlight*sunVisibility*(1.0-rainStrength*0.95)*length(rawAvg) + 1.5*length(rawAvg)*mix(vec3(0.16, 0.4, 1.),vec3(0.3,0.3,0.36),rainStrength)*(1.0-rainStrength*0.5) + 2.0*moonlight*moonVisibility*(1.0-rainStrength*0.95);
