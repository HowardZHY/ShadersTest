...
const float sunPathRotation = 0.0;  //[-45.0 -40.0 -30.0 -20.0 -15.0 -10.0 0.0 10.0 15.0 20.0 30.0 40.0 45.0]
bool gdepthMipmapEnabled = true; //[true false]

//number of refinements
const int maxf = 4; //[0 1 2 3 4 5 6]
//size of one step for raytracing algorithm
const float stp = 4.0f; //[0.0 0.5 1.0 1.5 2.0 2.5 3.0 4.0]
const float ref = 0.04;			//refinement multiplier
const float inc = 2.2;			//increasement factor at each step
float a0 = 0.25f; //[0.1 0.2 0.25 0.3 0.4 0.5]
float b0 = 0.2f; //[0.1 0.2 0.25 0.3 0.4 0.5]
float c0 = 0.10f; //[0.10 0.20 0.25 0.30 0.40 0.50]
...
const vec3 moonlight = vec3(1.2, 1.0, 1.0) * 0.005;
const vec3 moonlightS =vec3(1.2, 1.0, 1.0) * 0.01;
...
vec3 drawSun(vec3 fposition,vec3 color,float vis) {
    ...
    sun *= (1.0-rainStrength*0.996)*sunVisibility;
    vec3 sunlightB = mix(pow(sunlight,vec3(1.0))*2.2*20.,vec3(0.4,0.3,0.4),rainStrength*0.8)*(1.0+SdotU*2.0);
...
void main() {
	...
	if (land) {
		...
		if (reflective) {
			...
			sky = max(sky-2./16.0,0.0)*1.142857;
			sky *= sky;
			...
			vec3 sky_c = getSkyColor(reflectedVector*620.)*1.6;
		...
		vec3 fogC = getSkyColor(normalize(fragpos.xyz))*1.6*(0.7+0.3*tmult)*(1.33-rainStrength*0.8);

		if (isEyeInWater < 0.9)color = mix(color,fogC,fog*2.*(1.0-rainStrength*0.5));

		else color = mix(color*0.4,vec3(0.25,0.5,0.72)*rawAvg*0.1,1.-exp(-length(fragpos.xyz)/40));
	...
	vec4 rain = texture2D(composite,texcoord);
		...
		float torch_lightmap = 8.0 - min(rain.g/rain.r * 6.16,5.6);
		torch_lightmap = 0.2 / torch_lightmap / torch_lightmap - 0.0024;
