...
void main() {
	...
	float skyL = lmcoord.t*1.1-0.5/16.0*1.1;
	...
	const vec3 moonlight = vec3(1.6, 1.0, 1.0) * 0.006;
	...
	vec2 trCalc = min(abs(worldTime-vec2(23200.0,12000.0)),800.0);
	...
	float diffuse = (worldTime > 12000 && worldTime < 23200)? -NdotL : NdotL;
	...
	vec3 skyLight = mix(moonlight,mix(vec3(0.1,0.32,1.),vec3(0.3),rainStrength)*0.55+sunlightC*1.2*tr,visibility.x)*pow(skyL,3.)*0.3;
	color.rgb *= sunlightLight * mix(1.,coef,tr)*3.2+skyLight *0.8+ vec3(1.1,0.42,0.045)*torch_lightmap*0.66 + 0.002*min(skyL+6/16.,9/16.)*normalize(skyLight+0.0001);
	color.rgb *= 1.0;
