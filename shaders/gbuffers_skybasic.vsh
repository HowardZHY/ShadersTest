...
void main() {
	...
	float tr = clamp(min(min(distance(float(worldTime),23200.0),800.0),min(distance(float(worldTime),12700.0),800.0))/800.0-0.5,0.0,1.0)*2.0;
	
	const vec3 moonlight = vec3(1.2, 1.0, 1.4) * 0.0032;
...
float mcosS = max(cosS,0.0);
	...
	vec3 sun_ambient = bounced.w * (vec3(0.25,0.64,1.32)-rainStrength*vec3(0.1,0.48,1.17))*(1.0+rainStrength*7.0) + sunlight*(bounced.x + bounced.z)*(1.0-rainStrength*0.95);
